...
export default function Cobro() {
  // 1. CAMBIO: Estado inicial en null y nuevo estado de carga
  const [tasa, setTasa] = useState(null);
  const [loadingTasa, setLoadingTasa] = useState(true);

  // 2. NUEVO: Efecto para obtener la tasa de la API
  useEffect(() => {
    const obtenerTasa = async () => {
      try {
        const res = await fetch('/api/tasa');
        const data = await res.json();
        // Asumiendo que tu API devuelve { tasa: valor }
        setTasa(data.tasa);
      } catch (error) {
        console.error("Error obteniendo tasa:", error);
        setTasa(295.50); // Valor de respaldo (fallback) por si la API falla
      } finally {
    ...
    // PASO 2: DETALLES Y PAGO
    {
      form: {
        ...
        render: ({ values, onNext, onBack }) => {
          ...
          // El cálculo ahora usa la tasa de la API
  ...
  // 3. BLOQUEO DE CARGA: Si la tasa aún no llega, mostramos un aviso
  if (loadingTasa) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-primary animate-pulse font-bold">Obteniendo tasa de cambio...</div>
      </div>
    );
  }
