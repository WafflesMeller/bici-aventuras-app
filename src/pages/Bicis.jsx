...
const ModalSeleccionCliente = ({ isOpen, onClose, onConfirm, bikeName }) => {
  ...
  const fetchVentasConCupos = async () => {
    ...
    const { data: ventas } = await supabase
      ...
      .select("id, nombre_cliente, tiempo_alquiler, created_at, cantidad_bicicletas")
      .eq("pagado", true)
      .gte("created_at", hace12h)
      .order("created_at", { ascending: false });

    if (!ventas) {
        setVentasDisponibles([]); setLoading(false); return;
    }
    ...
    const ventasFiltradas = ventas.map(venta => {
        ...
        const totalUsados = usos ? usos.filter(u => u.venta_id === venta.id).length : 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-[#1a1a1a] border border-white/10 w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-4 border-b border-white/10 flex justify-between items-center">
          <h3 className="text-white font-bold text-lg">Configurar {bikeName}</h3>
          <button onClick={onClose} className="text-white/50 hover:text-white"><X size={20}/></button>
        </div>
        
        <div className="p-4 max-h-[60vh] overflow-y-auto space-y-2">
          {loading ? (
            <div className="text-center text-primary py-4">Buscando cupos...</div>
          ) : ventasDisponibles.length === 0 ? (
            <div className="text-center text-white/30 py-4">Sin clientes con cupos</div>
          ) : (
            ventasDisponibles.map((venta) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-white group-hover:text-primary">{venta.nombre_cliente}</span>
                  <div className="flex flex-col items-end">
                      <span className="text-xs bg-white/10 px-2 py-1 rounded text-white/70">{venta.tiempo_alquiler}</span>
                      <span className="text-[10px] text-green-400 font-bold mt-1">{venta.cupos_restantes} cupo(s)</span>
                  </div>
                </div>
              </button>
            ))
        ...
        <div className="p-4 bg-white/5 border-t border-white/10 grid grid-cols-2 gap-2">
            <button onClick={() => onConfirm("10 min", null)} className="py-2 bg-white/5 hover:bg-primary hover:text-black rounded-lg text-xs font-bold border border-white/10 transition-colors">10 Min (Libre)</button>
            <button onClick={() => onConfirm("20 min", null)} className="py-2 bg-white/5 hover:bg-primary hover:text-black rounded-lg text-xs font-bold border border-white/10 transition-colors">20 Min (Libre)</button>
        </div>
      </div>
    </div>
  );
...
const BiciCard = ({ bici, info, preData, now, onAbrirModal, onIniciarReal, onCancelarPre, onTerminar, onAgregar, onTogglePausa }) => {
  ...
  const progreso = ocupada && !tiempoAgotado && !pausada
  ...
  return (
    <div className={`relative rounded-3xl overflow-hidden border transition-all duration-300 
      ${preseleccionada ? "bg-primary/5 border-primary shadow-[0_0_15px_rgba(0,255,127,0.3)] scale-105 z-10" : 
        !ocupada ? "bg-white/5 border-white/10" : 
        pausada ? "bg-yellow-500/10 border-yellow-500/50" :
        tiempoAgotado ? "bg-red-950/80 border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.6)] animate-pulse" : 
      ...
      <div className="relative z-10 p-4">
        ...
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-full ${ocupada ? (tiempoAgotado ? "bg-red-500 text-black" : pausada ? "bg-yellow-500 text-black" : "bg-primary text-black") : preseleccionada ? "bg-primary text-black" : "bg-white/15 text-white/50"}`}>
              {tiempoAgotado ? <AlertCircle size={14} strokeWidth={3} /> : <Bike size={14} strokeWidth={2.5} />}
            </div>
            <div>
              <span className={`block font-black text-base leading-none ${ocupada || preseleccionada ? "text-white" : "text-white/60"}`}>{bici}</span>
              {(info?.cliente_nombre || preData?.cliente_nombre) && (
                 <span className="text-[10px] text-white/70 flex items-center gap-1 mt-0.5">
                   <User size={8}/> {info?.cliente_nombre || preData?.cliente_nombre}
                 </span>
              )}
            </div>
          </div>
          ...
          {ocupada && !tiempoAgotado && (
            <button onClick={() => onTerminar(info.id)} className="p-2 -mr-2 -mt-2 text-white/40 hover:text-red-500 bg-black/20 rounded-full transition-colors">
              <Trash2 size={16} />
            </button>
          )}
        </div>
        ...
        <div>
          ...
          {!ocupada && !preseleccionada && (
            <button 
              ...
              className="w-full py-3 mt-2 bg-white/5 hover:bg-white/10 active:bg-primary active:text-black border border-white/10 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-2"
            >
          ...
          {preseleccionada && (
            <div className="mt-2 animate-fade-in">
                <div className="text-center mb-2">
                    <span className="text-3xl font-mono font-black text-primary/50">{preData.tiempoTexto}</span>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => onCancelarPre(bici)} className="flex-1 py-2 bg-red-500/20 text-red-500 rounded-lg text-xs font-bold hover:bg-red-500/30">CANCELAR</button>
                    <button onClick={() => onIniciarReal(bici)} className="flex-[2] py-2 bg-primary text-black rounded-lg text-xs font-bold hover:brightness-110 flex items-center justify-center gap-1 shadow-lg shadow-primary/20">
          ...
          {ocupada && (
            <div className="text-center pt-1">
              <div className={`text-3xl font-mono font-black tracking-tighter mb-2 drop-shadow-md transition-colors 
                ${pausada ? "text-yellow-400" : tiempoAgotado ? "text-red-500 scale-110 duration-500" : "text-white"}`}>
                {pausada ? "PAUSA" : formatoTiempo(tiempoRestanteMs)}
              </div>
              
              {pausada && <div className="text-xs text-yellow-400/70 font-mono mb-2">Restan: {formatoTiempo(tiempoRestanteMs)}</div>}

              {tiempoAgotado ? (
                <div className="space-y-2 animate-fade-in">
                    <div className="text-red-400 text-[10px] font-bold uppercase tracking-widest mb-1">¡TIEMPO FINALIZADO!</div>
                    <button 
                        onClick={() => onTerminar(info.id)}
                        className="w-full py-3 bg-red-500 hover:bg-red-400 text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-[0_0_15px_rgba(239,68,68,0.5)] transition-all transform hover:scale-105 flex items-center justify-center gap-2"
                    >
                        <CheckCircle2 size={16} /> RECIBIR BICI
                    </button>
                    <button 
                        onClick={() => onAgregar(info.id, fechaFin, 5)} 
                        className="w-full py-1.5 bg-black/40 text-white/50 hover:text-white rounded text-[10px] font-bold flex justify-center items-center gap-1"
                    >
                        <Plus size={10} /> Agregar 5 min
                    </button>
                </div>
              ) : (
                <div className="flex gap-2">
                   <button 
                     ...
                     className={`flex-1 py-1.5 rounded text-[10px] font-bold flex justify-center items-center gap-1 border transition-all
                       ${pausada 
                           ? "bg-green-500/20 text-green-400 border-green-500/30 hover:bg-green-500/30" 
                           : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20 hover:bg-yellow-500/20"}`}
                   >
                     {pausada ? <><PlayCircle size={12}/> REANUDAR</> : <><PauseCircle size={12}/> PAUSAR</>}
                   </button>
                   
                   {!pausada && (
                       <button onClick={() => onAgregar(info.id, fechaFin, 5)} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-white/50 hover:text-white rounded text-[10px] font-bold flex justify-center items-center gap-1 border border-white/5">
                         <Plus size={10} /> +5 min
                       </button>
                   )}
                </div>
...
export default function BicisPage() {
  ...
  const handleAgregarTiempo = async (idRegistro, fechaFinActual, minutosExtra) => {
  ...
  const { enPista, disponibles } = useMemo(() => {
    const activeMap = {};
    pistaData.forEach(item => { activeMap[item.bicicleta] = item; });
